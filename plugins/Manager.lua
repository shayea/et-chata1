...
if not v.print_name then
    ...
    local file = io.open("./Home/chat/lists/supergroups/"..cb_extra.receiver..".txt", "w")
    file:write(text)
    file:flush()
    file:close()
    send_document(cb_extra.receiver,"./Home/chat/lists/supergroups/"..cb_extra.receiver..".txt", ok_cb, false)
...
if not v.print_name then
    ...
    local file = io.open("./Home/chat/lists/supergroups/kicked/"..cb_extra.receiver..".txt", "w")
    file:write(text)
    file:flush()
    file:close()
    send_document(cb_extra.receiver,"./Home/chat/lists/supergroups/kicked/"..cb_extra.receiver..".txt", ok_cb, false)
	--send_large_msg(cb_extra.receiver, text)
end
...
function show_supergroup_settingsismod(msg, target)
 	if not is_momod(msg) then
    	return
  	end
	local data = load_data(_config.moderation.data)
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'بلوک' or matches[1] == 'اخراج' and is_momod(msg) then
			if type(msg.reply_id) ~= "nil" then
				...
				get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'بلوک' or matches[1] == 'اخراج' and string.match(matches[2], '^%d+$') then
    ...
    return unlock_group_inline(msg, data, target)
		...
		if matches[1] == 'تنظیمات' and is_momod(msg) then
			...
			return show_supergroup_settingsismod(msg, target)
		end
